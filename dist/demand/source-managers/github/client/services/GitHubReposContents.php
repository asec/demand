...
class GitHubReposContents extends GitHubService
{
	 ...
	 * @return GitHubReadmeContent
	 */
	public function getFile($owner, $repo, $filename, $ref = null)
	{
		$data = array();
		if(!is_null($ref))
			$data['ref'] = $ref;

		return $this->client->request("/repos/$owner/$repo/contents/$filename", 'GET', $data, 200, 'GitHubReadmeContent');
	}
	 ...
	 * @return GitHubReadmeContent
	 */
	public function getDir($owner, $repo, $path, $ref = null)
	{
		$data = array();
		if(!is_null($ref))
			$data['ref'] = $ref;

		return $this->client->request("/repos/$owner/$repo/contents/$path", 'GET', $data, 200, 'GitHubReadmeContent', true);
	}
